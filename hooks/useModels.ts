...
export function useModels(): UseModelsResult {
  ...
  const { apiKey, baseUrl, proxyUrl, activeProfile } = useApiConfig();
  const customModels = activeProfile?.customModels;

  // 拉取结果统一经此合并当前节点的自定义模型，保证设置里手工补的模型始终可选
  const applyModels = useCallback(
    (list: ModelInfo[]) => setModels(mergeCustomModels(list, customModels)),
    [customModels]
  );
  ...
  }, [apiKey, baseUrl, proxyUrl, applyModels]);
