  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (baseUrlInput.trim()) {
        headers["x-base-url"] = baseUrlInput.trim();
      }
      if (proxyUrlInput.trim()) {
        headers["x-proxy-url"] = proxyUrlInput.trim();
      }
  ...
  return (
    <header className="flex items-center justify-between px-6 py-3 border-b border-border bg-card/80 backdrop-blur-sm">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-lg font-semibold text-foreground">AI Image Gen</h1>
      </div>

      <div className="flex items-center gap-3">
        {apiKey && (
          <div className="flex items-center gap-1.5 text-xs text-green-500" title="API Key 已配置">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  第三方中转请填写服务商提供的地址，留空默认使用 OpenAI 官方
                </p>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">
                  如果服务端无法直连 API，请填写代理地址（支持 HTTP/SOCKS5）
                </p>
              </div>
